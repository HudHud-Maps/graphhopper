...
public class NavigateResource {
    ...
    public NavigateResource(
        ...
        resolverMap = config
            .asPMap()
            .getObject("profiles_mapbox", new HashMap<>());
    ...
    public Response doGet(
        ...
        @QueryParam("voice_instructions") @DefaultValue(
            "false"
        ) boolean voiceInstructions,
        @QueryParam("banner_instructions") @DefaultValue(
            "false"
        ) boolean bannerInstructions,
        @QueryParam("roundabout_exits") @DefaultValue(
            "false"
         ...
         */
        if (!geometries.equals("polyline6")) throw new IllegalArgumentException(
            "Currently, we only support polyline6"
        );
        if (!enableInstructions) throw new IllegalArgumentException(
            "Currently, you need to enable steps"
        );
        if (!roundaboutExits) throw new IllegalArgumentException(
            "Roundabout exits have to be enabled right now"
        );
        if (!voiceInstructions) throw new IllegalArgumentException(
            "You need to enable voice instructions right now"
        );
        if (!bannerInstructions) throw new IllegalArgumentException(
            "You need to enable banner instructions right now"
        );

        double minPathPrecision = 1;
        if (overview.equals("full")) minPathPrecision = 0;
        ...
        String ghProfile = resolverMap.getOrDefault(
            mapboxProfile,
            mapboxProfile
        );
        List<GHPoint> requestPoints = getPointsFromRequest(
            httpReq,
            mapboxProfile
        );
        ...
        if (
            favoredHeadings.size() > 0 &&
            favoredHeadings.size() != requestPoints.size()
        ) {
            throw new IllegalArgumentException(
                "Number of bearings and waypoints did not match"
            );
        }
        ...
        if (!ghResponse.hasErrors() && favoredHeadings.size() > 0) {
            ...
            if (
                ghResponse.getBest().getDistance() !=
                noHeadingResponse.getBest().getDistance()
            ) {
        ...
        if (ghResponse.hasErrors()) {
            ...
            return Response.status(422)
                .entity(
                    NavigateResponseConverter.convertFromGHResponseError(
                        ghResponse
                    )
                )
            ...
            logger.info(logStr);
            return Response.ok(
                NavigateResponseConverter.convertFromGHResponse(
                    ghResponse,
                    translationMap,
                    locale,
                    config
    ...
    private GHResponse calcRoute(
        List<Double> headings,
        List<GHPoint> requestPoints,
        String profileStr,
        String localeStr,
        ...
        if (headings.size() > 0) request.setHeadings(headings);
     ...
     */
    private List<GHPoint> getPointsFromRequest(
        HttpServletRequest httpServletRequest,
        String profile
    ) {
        String url = httpServletRequest.getRequestURI();
        String urlStart = "/navigate/directions/v5/gh/" + profile + "/";
        if (!url.startsWith(urlStart)) throw new IllegalArgumentException(
            "Incorrect URL " + url
        );
    ...
    static List<Double> getBearing(String bearingString) {
        if (
            bearingString == null || bearingString.isEmpty()
        ...
        for (int i = 0; i < bearingArray.length; i++) {
            ...
            if (singleBearing.isEmpty()) {
                ...
                if (!singleBearing.contains(",")) {
                    throw new IllegalArgumentException(
                        "You passed an invalid bearings parameter " +
                        bearingString
                    );
                }
